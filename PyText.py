# Tutorial link
# https://www.youtube.com/channel/UCxPWtz5N--X3IyYJ13Zr99A
...
class Menubar:
    def __init__(self,parent):
        ...
        file_dropdown.add_command(label='Exit',accelerator='Ctrl+W',command=parent.master.destroy)

        

        about_dropdown=tk.Menu(menubar,font=font_specs,tearoff=0)
        about_dropdown.add_command(label="Release Notes",command=self.show_release)
        about_dropdown.add_separator()
        about_dropdown.add_command(label="About",command=self.show_about)
        
        menubar.add_cascade(label="File",menu=file_dropdown)
        menubar.add_cascade(label="About",menu=about_dropdown)
    def show_about(self):
        box_title='About PyText'
        box_message='Simple Python Text Editor'
...
class PyText:
    ...
    def open_file(self,*args):
        self.filename=filedialog.askopenfilename(
            ...
            filetypes=[('All Files',"*.*"),("Text Files","*.txt"),('Python Files','*.py'),('Markdown docs','*.md'),('Javascript Files','*.js'),('html docs','*.html'),('css docs','*.css')]
        )
        print(self.filename)
        if self.filename:
            self.textarea.delete(1.0,tk.END)
    ...
    def bind_shortcuts(self):
        ...
        self.textarea.bind('<Control-S>',self.save_as)#ctrl+shift+s=<Control-S>  #capital 'S'
